...
export async function fetchDelta3Status(): Promise<Delta3Status> {
  const response = await fetch("/api/delta3/status", { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`DELTA 3 status request failed: ${response.status}`);
  }
...
export async function fetchDelta3AuxControlCommandLogsPage({
  limit,
  offset,
  from,
  to
  ...
  from?: string;
  to?: string;
}): Promise<Delta3AuxControlCommandLogsPage> {
  const params = new URLSearchParams({
    limit: String(limit),
    offset: String(offset)
  });
  ...
  const response = await fetch(`/api/delta3/aux-commands?${params.toString()}`, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`DELTA 3 Plus auxiliary command logs request failed: ${response.status}`);
  }
  return response.json();
}

export async function fetchNightChargeSummariesPage({
  limit,
  offset,
  from,
  to
  ...
  from?: string;
  to?: string;
}): Promise<NightChargeDailySummariesPage> {
  const params = new URLSearchParams({
    limit: String(limit),
    offset: String(offset)
  });
  ...
  const response = await fetch(`/api/night-charge/summaries?${params.toString()}`, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`night charge summaries request failed: ${response.status}`);
  }
  return response.json();
}
